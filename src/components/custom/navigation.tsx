...
import { Home, Camera, Package, BookOpen, User, ShoppingCart } from 'lucide-react';
...
export function Navigation() {
  ...
  const navItems = [
    ...
    ...(isPremium ? [{ href: '/shopping-cart', icon: ShoppingCart, label: 'Carrinho', isCartMenu: true }] : []),
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-t border-gray-200 dark:border-gray-800 shadow-lg">
      <div className="max-w-screen-xl mx-auto">
        <div className={`grid ${isPremium ? 'grid-cols-6' : 'grid-cols-5'} h-16 sm:h-20`}>
          {navItems.map((item) => {
